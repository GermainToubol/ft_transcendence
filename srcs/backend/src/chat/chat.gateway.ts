...
export class ChatGateway {
  ...
  async handleConnection(client: UserSocket) {
    ...
    if (!channels || !chatter)
      return
    channels.filter(function isChannelUser(channel: ChatChannel): boolean {
      return this.chatService.isChannelUser(chatter, channel)
    })
      .filter(function isNotBanned(channel: ChatChannel): boolean {
        return !this.chatService.isBannedFromChannel(chatter, channel)
      })
      .forEach(function addUserToChannel(channel: ChatChannel) {
  ...
  async handleChannelCreation(client: UserSocket, payload: ChatChannelDto) {
    ...
    if (await this.chatService.addChannelUser(owner, payload.password, channel.id).catch(() => false)) {
