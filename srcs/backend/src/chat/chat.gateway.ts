...
export class ChatGateway {
    ...
    async handleBlockChatter(client: UserSocket, payload: BanChatterDto) {
        ...
        if (await this.chatService.blockUser(user, blocked)) {
            client.emit('addBlock', { login: blocked.user.login, name: blocked.user.usual_full_name })
        }
    }
    ...
    async handleUnlockChatter(client: UserSocket, payload: BanChatterDto) {
        ...
        if (!user || !blocked)
            return
        if (await this.chatService.unblockUser(user, blocked)) {
            client.emit('popBlock', { login: blocked.user.login, name: blocked.user.usual_full_name })
        }
    }
