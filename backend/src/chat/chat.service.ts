...
export class ChatService {
    ...
    async banChatterFromChannel(admin: Chatter, banned: Chatter, channelId: number): Promise<boolean> {
        const channel: ChatChannel = await this.getChannelById(
            channelId,
            { bannedUsers: true, channelAdmins: true, owner: true })
            .catch(() => null)
        if (!channel)
            return false
        if (this.channelService.isChatterAdminFromChannel(admin, channel) && banned.id !== channel.owner.id) {
    ...
    async muteChatterFromChannel(admin: Chatter, user: Chatter, channelId: number): Promise<boolean> {
        const channel: ChatChannel = await this.getChannelById(
            channelId,
            { channelAdmins: true, mutedUsers: true, owner: true })
            .catch(() => null)
        if (!channel)
            return false
        if (this.channelService.isChatterAdminFromChannel(admin, channel) && user.id !== channel.owner.id) {
